...
升級程序會取消註冊現有的外掛程式、並部署僅與vSphere 7.0U1及更新版本相容的外掛程式。
...
.. 在上傳快顯畫面上、瀏覽至您下載的位置 `.iso` 檔案、然後按一下 `.iso` 檔案映像、然後按一下*「Open*（開啟*）」。
檔案會上傳至資料存放區。
.. 瀏覽回要升級的VM、然後按一下* Summary（摘要）*索引標籤。
...
... 在內容欄中、瀏覽至 `.iso` 您上傳的檔案、請確定「檔案類型」欄位中已選取「ISO映像」、然後按一下「*確定*」。
